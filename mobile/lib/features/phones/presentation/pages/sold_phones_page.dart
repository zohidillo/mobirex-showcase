...
class SoldPhonesPage extends ConsumerStatefulWidget {
  const SoldPhonesPage({super.key});

  @override
  ConsumerState<SoldPhonesPage> createState() => _SoldPhonesPageState();
}

class _SoldPhonesPageState extends ConsumerState<SoldPhonesPage> {
  ...
  void _onScroll() {
    if (_scrollCtrl.position.pixels >=
      ...
      ref.read(soldPhonesProvider.notifier).loadMore();
    }
  }

  void _apply(PhoneFilter filter) =>
      ref.read(soldPhonesProvider.notifier).applyFilter(filter);

  Future<void> _confirmReturn(PhoneModel phone) async {
    final ok = await showAppConfirmDialog(
      context: context,
      title: 'Telefonni qaytarish',
      content: AppDialogText(
        emphasis: phone.name,
        text: ' sotilmaganlar ro\'yxatiga qaytarilsinmi?',
      ),
    ...
    if (ok == true) {
      final err = await ref
          .read(soldPhonesProvider.notifier)
          .returnPhone(phone.id);
      if (err != null && mounted) {
  ...
  Widget build(BuildContext context) {
    final state = ref.watch(soldPhonesProvider);
    ...
    return PopScope(
      ...
      child: Scaffold(
        appBar: VelmoraAppBar(
          subtitle: 'Sotilgan telefonlar',
          showDrawer: true,
          actions: [
            AppHeaderButton(
              icon: Icons.phone_android,
              onPressed: () => context.go('/phones/unsold'),
              tooltip: 'Sotilmagan telefonlar',
            ),
          ],
  ...
  Widget _buildList(PhoneListState state) {
    ...
    if (state.error != null && state.phones.isEmpty) {
      return ErrorView(
        ...
        onRetry: () =>
            ref.read(soldPhonesProvider.notifier).load(refresh: true),
      );
    }
    if (state.phones.isEmpty) {
      return EmptyState(
        message: 'Bu davrda sotilgan telefonlar yo\'q',
    ...
    return RefreshIndicator(
      ...
      child: ListView.separated(
        ...
        itemBuilder: (ctx, i) {
          ...
          final canReturn = isCurrentMonth(phone.soldAt);
          return SoldPhoneCard(
            phone: phone,
            canReturn: canReturn,
            onReturn: canReturn ? () => _confirmReturn(phone) : null,
          );
        },
      ),
    );
